...
- Architecture boundaries with disjoint `wraps` memberships must have disjoint final frames whenever a quality profile is active. Shared-member boundaries may overlap to express orthogonal scopes, and valid nested boundaries receive deterministic breathing room, but no two structural frames may share a positive-length painted border segment.
